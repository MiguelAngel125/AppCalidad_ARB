...
function Principal() {
  ...
  DBCalidad.forEach((item) => {
    ...
    let calidad = ((t5 * 5 + t4 * 4 + t3 * 3 + t2 * 2 + t1) / sumTotal).toFixed(
      2
    );
    let cat1 = (((sumTotal - t3 - t2 - t1) * 100) / sumTotal)
      .toFixed(0)
      .toString();
  ...
  const Productor = data_1.map((x) => {
    let productor = "";
    productor = x.productor;
    let value = "";
    return (value = x.productor);
  });
  const filterProdRepetido = Productor.filter(
    (x, index) => Productor.indexOf(x) === index
  );
  const filterNombre = filterProdRepetido.map((x) => {
    let nombre = "";
    let resumenNombre = "";
    nombre = x;
    switch (nombre) {
      case "JORGE A. MARTINEZ ZUCCARDI":
        resumenNombre = "JMZ";
        break;
      case "ERNESTO FERNANDEZ E HIJOS":
        resumenNombre = "EHF";
        break;
      case "Forestal San Ignacio S.R.L":
        resumenNombre = "DIB";
        break;
      case "Sucesores de SALOMON JALIL s.r.l.":
        resumenNombre = "JLI";
        break;
      case "Ledesma S.A.A.I.":
        resumenNombre = "LED";
        break;
      default:
        nombre = nombre;
    }
    return { text: resumenNombre, value: nombre };
  });

  const Finca = data_1.map((x) => {
    let finca = "";
    finca = x.finca;
    let value = "";
    return (value = x.finca);
  });
  const filterFincaRepetida = Finca.filter(
    (x, index) => Finca.indexOf(x) === index
  );
  const filterFinca = filterFincaRepetida.map((x) => {
    let finca = "";
    finca = x;
    return { text: finca, value: finca };
  });

  const UP = data_1.map((x) => {
    let up = "";
    up = x.up;
    let value = "";
    return (value = x.up);
  });
  const filterUpRepetida = UP.filter((x, index) => UP.indexOf(x) === index);
  const filterUP = filterUpRepetida.map((x) => {
    let up = "";
    up = x;
    return { text: up, value: up };
  });

  const Calibre = data_1.map((x) => {
    let calibre = "";
    calibre = x.calibre;
    let value = "";
    return (value = x.calibre);
  });
  const filterCalibreRepetido = Calibre.filter(
    (x, index) => Calibre.indexOf(x) === index
  );
  const filterCalibre = filterCalibreRepetido.map((x) => {
    let calibre = "";
    calibre = x;
    return { text: calibre, value: calibre };
  });
    ...
    <>
      <div className="table-responsive">
        <nav style={{color: "white",backgroundColor: "#1C2B36",height: 55,fontSize: 25,display: "flex",justifyContent: "space-around",flexDirection: "row",}}>
          ...
          <button className="btn" style={estiloBotonActualizar} onClick={(e) => {
              if (
                window.confirm(
                  "Hay " + index_1 + " Registros. Desea continuar?"
                )
              )
                mostrardata();
            }}
          >{iconActualizar}</button>
